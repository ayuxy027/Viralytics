          ...
          <div className="flex justify-between items-center h-14 sm:h-16 md:h-20">
            ...
            >
              <span className={`bg-clip-text text-transparent bg-gradient-to-r ${
                darkMode ? 'from-dark-primary to-dark-secondary' : 'from-light-primary to-light-secondary'
              }`}>
                Viralytics
              </span>
            </motion.a>
  
            {/* Central Nav Links - Only visible when authenticated */}
            {isAuthenticated && (
            ...
            <div className="flex items-center space-x-1 sm:space-x-2 md:space-x-3 lg:space-x-4">
              ...
              <motion.button
                ...
                className={`p-1 sm:p-2 rounded-lg sm:rounded-xl transition-all duration-300 ${
                  darkMode ? 'hover:bg-dark-primary/20 text-dark-primary' : 'hover:bg-light-primary/20 text-light-tertiary'
                }`}
              >
                {darkMode ? (
              ...
              {isAuthenticated ? (
                <>
                    ...
                    <AnimatePresence>
                      {showNotifications && (
                        <motion.div
                          ...
                          animate="visible"
                          exit="exit"
                          className={`absolute right-0 mt-2 w-60 sm:w-64 md:w-72 rounded-xl shadow-lg border backdrop-blur-md ${
                            darkMode
                          ...
                          <div className="p-2">
                            {notifications.map(notification => (
                              <motion.div
                                ...
                                className={`flex flex-col p-2 sm:p-3 rounded-lg transition-all duration-300 ${
                                  darkMode ? "hover:bg-dark-primary/10" : "hover:bg-light-primary/10"
                                }`}
                              >
                  ...
                  <div className="relative" ref={userMenuRef}>
                    <motion.button
                      ...
                      className={`flex items-center p-1 sm:p-2 rounded-lg sm:rounded-xl transition-all duration-300 ${
                        darkMode ? 'hover:bg-dark-primary/20' : 'hover:bg-light-primary/20'
                      }`}
                    >
                      <UserProfile />
                    </motion.button>
                    ...
                    <AnimatePresence>
                      {showUserMenu && (
                        <motion.div
                          ...
                          animate="visible"
                          exit="exit"
                          className={`absolute right-0 mt-2 w-40 sm:w-44 md:w-48 rounded-xl shadow-lg border backdrop-blur-md ${
                            darkMode
                              ? "bg-dark-background/95 border-dark-primary/20"
                              : "bg-light-background/95 border-light-primary/20"
                          }`}
                        >
                          <div className="p-2">
                            <motion.button
                              ...
                              className={`flex items-center w-full p-2 sm:p-3 text-sm sm:text-base rounded-lg transition-all duration-300 ${
                                darkMode
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={handleLogin}
                  className={`px-3 sm:px-4 md:px-6 py-1.5 sm:py-2 text-sm sm:text-base font-medium rounded-lg sm:rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl ${
                    darkMode
              ...
              <motion.button
                ...
                className={`p-1 sm:p-2 rounded-lg sm:rounded-xl md:hidden transition-all duration-300 ${
                  darkMode ? 'hover:bg-dark-primary/20 text-dark-primary' : 'hover:bg-light-primary/20 text-light-tertiary'
                }`}
              >
                {isMobileMenuOpen ? (
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              animate="visible"
              exit="exit"
              className={`fixed inset-y-0 right-0 w-48 sm:w-56 md:w-64 shadow-xl md:hidden ${
                darkMode
